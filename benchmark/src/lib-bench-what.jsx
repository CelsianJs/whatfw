/**
 * Library Benchmark — What Framework (via what-react compat)
...
// Create 20 fields for more measurable form operations
const defaultFields = {};
...
function TableBench() {
  ...
  // Render actual table rows (this is where render cost shows!)
  return (
    <div>
...
function App() {
  return (
    <div style={{ padding: 16, fontFamily: 'monospace', fontSize: 12 }}>
      <h3>What Framework — Library Benchmark</h3>
      <ZustandBench />
      <HookFormBench />
      <TableBench />
...
window.addEventListener('message', (e) => {
  ...
  try {
    ...
    bench[method](...(args || []));
    // Respond after a microtask flush so signals/React can process
...
window.addEventListener('load', () => {
  setTimeout(() => {
    if (window.parent !== window) {
      window.parent.postMessage({ type: 'bench-ready', framework: 'what' }, '*');
    }
  }, 500);
});
